...
int main(int argc, const char* argv[]) {
    ...
    camera.addComponent<Nickel2::CameraComponent>(&window, 90.0f, 1.0f, 1000.0f, NK_EULER_OVERRIDE);
    ...
    while (!window.shouldClose()) {
        ...
        if (window.input->getKey(NK_KEY_R))
...
class FirstPersonCamera : public Nickel2::Entity {
    ...
    public:
        ...
        FirstPersonCamera(Nickel2::Scene* scene, Nickel2::Window* window, float fov, float sensitivity, float nearPlane, float far)
            ...
            addComponent<Nickel2::CameraComponent>(window, fov, nearPlane, far, NK_EULER_OVERRIDE);
        }
        ...
        void onUpdate(float deltaTime) override {
            if (window->input->getKey(NK_KEY_LEFT_ALT)) {
                window->input->setCursorVisible(true);
                return;
            }

            if (!glfwGetWindowAttrib(window->getGLFWWindow(), GLFW_FOCUSED)) return;
            float speed = window->input->getKey(NK_KEY_LEFT_CONTROL) ? sprintSpeed : normalSpeed;
            ...
            if (window->input->getKey(NK_KEY_W)) {
            ...
            if (window->input->getKey(NK_KEY_S)) {
                transform->translate(speed * -glm::vec3(glm::cos(glm::radians(camera->yaw)), 0.0f, glm::sin(glm::radians(camera->yaw))) * window->deltaTime * 10.0f);
            }

            if (window->input->getKey(NK_KEY_A)) {
                transform->translate(speed * -glm::normalize(glm::cross(camera->front, camera->up)) * window->deltaTime * 10.0f);
            }

            if (window->input->getKey(NK_KEY_D)) {
                transform->translate(speed * glm::normalize(glm::cross(camera->front, camera->up)) * window->deltaTime * 10.0f);
            }

            if (window->input->getKey(NK_KEY_SPACE)) {
                transform->translate(speed * camera->up * window->deltaTime * 10.0f);
            }

            if (window->input->getKey(NK_KEY_LEFT_SHIFT)) {
                transform->translate(speed * -camera->up * window->deltaTime * 10.0f);
            }
