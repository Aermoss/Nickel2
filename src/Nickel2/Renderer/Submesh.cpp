...
namespace Nickel2 {
    ...
    SubMesh::SubMesh(std::vector <Vertex> vertices, std::vector <uint32_t> indices, Material& material, Transform* parent)
    ...
    std::vector <uint32_t> SubMesh::getIndices() {
        return indices;
    }

    void SubMesh::render(Shader* shader, bool useTexture) {
        glEnable(GL_DEPTH_TEST);
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        shader->use();
        shader->setUniform4fv("albedoDefault", (float*) glm::value_ptr(material.albedo));
        ...
        shader->setUniformMatrix3fv("inverseModel", (float*) glm::value_ptr(glm::transpose(glm::inverse(glm::mat3(matrix)))));
