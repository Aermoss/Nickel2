...
float shadowCalculationPoint(int lightIndex, vec3 position) {
    if (lightIndex != 0 || enableShadowsPoint == 0) return 0.0f;
...
float shadowCalculationDirectional(vec3 normal) {
    if (enableShadowsDirectional == 0) return 0.0f;
...
void main() {
    ...
    for (int i = 0; i < pointLightCount; i++) {
        ...
        float attenuation = (pointLightColors[i].a * 100.0f) / (distance * distance);
        vec3 radiance = pointLightColors[i].xyz * attenuation;
        float NDF = distributionGGX(N, H, roughness);
        float G = geometrySmith(N, V, L, roughness);
        vec3 F = fresnelSchlick(max(dot(H, V), 0.0f), F0);
        vec3 numerator = NDF * G * F;
